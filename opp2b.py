...
Z_values = np.zeros(len(interval)-1)
...
for n in [5,15,50]:
    ...
    for Z,name in zip(Zs,Z_names):
        ...
        for k in range(int(len(interval))-1):
            ...
            Z_values[k] = (np.sum(Z<interval[k+1])-np.sum(Z<interval[k]))/float(len(Z))
        ...
        plt.title("Percentile Plot for %s Distribution for n = %g"%(name,n))
        plt.xlabel("Percentile")
        plt.ylabel("Z")
        #plt.show()
        plt.savefig("/home/daniel/Dokumenter/Skole/Stk1100/Oblig2/percent%s%g"%(name,n))
        plt.clf()

        print "Sannsynlighetene for en standarnormalfordelt variabel:"
        print standard
        print "Relativ frekvenser for Z_n for %s fordeling:"
        print Z_values
        print "Forskjellen mellom Z_n og en standarnormalfordelt variabel:"
        print np.abs(Z_values - standard)
        print "                         "
        ...
        plt.ylabel("Count")
        #plt.show()
        plt.savefig("/home/daniel/Dokumenter/Skole/Stk1100/Oblig2/histo%s%g"%(name,n))
        plt.clf()
